...
这个绝妙构想击中了我，但没有击中我的老板 —— 很遗产，我司做产品功能从来都不通过产品需求文档来传达信息，富含禅意的AC123、心领神会的眼神交流和口口相传更适合华夏文明孕育下的敏捷宝宝食用。

不管怎么说，我基于这个想法做了个最基础的多角色协同框架demo，我将其命名为SynergyAI（协智AI），其核心突破为：
...
## Codex Skill：多角色 PRD 协作评审

仓库同时提供了一个纯原生 Codex Skill：`multi-agent-prd-review`。它不依赖 `openai` Python SDK，也不要求额外配置 LLM Token；模型调用和 subagent 编排由 Codex 运行时负责。

Skill 支持以下内置角色：

- 产品经理：用户价值、需求范围、优先级和验收标准
- 软件工程师：架构、接口、数据流、可行性和可靠性
- UX 设计师：交互流程、信息架构、反馈和可访问性
- 安全与隐私专家：认证授权、敏感数据、威胁和隐私控制
- QA / 测试工程师：边界条件、异常流程、可验证性和验收覆盖

默认启用产品经理、软件工程师和 UX 设计师，也可以在请求中指定其他角色和讨论轮数。例如：

```text
请使用 $multi-agent-prd-review 审查 sample_document.md。
启用产品经理、软件工程师、UX、安全和 QA，最多讨论 5 轮。
```

工作流程为：独立角色初审、顺序圆桌讨论、独立共识判定、全文重写和一致性审计。每个角色拥有独立上下文；后续角色只接收前序角色公开的结构化观点。Codex 会在 CLI 中输出完整的初审、公开立场、逐条回应、提案、未决问题、上下文交接和共识证据，便于人工复核。隐藏推理和私有上下文不会输出。

Skill 默认最多讨论 5 轮，可指定 1–15 轮。达成共识后，编辑器会直接修改原文对应章节，删除被替代的旧规则，并由独立审计角色检查全文冲突、过期要求、重复规则和验收标准不一致。它不会在文末追加一个覆盖章节来掩盖前后矛盾。

运行完成后，Skill 会生成：

```text
review-output/optimized.md  # 通过一致性审计的完整优化文档
review-output/report.json   # 角色输出、轮次、共识证据、决策映射和审计结果
```
